...
- **Staging Sync** - 大量外部資料以「暫存表＋單一交易原子切換」做全量同步，封頂記憶體同時保住原子性（設計文件：`docs/staging-sync/design.md`；schema／advisory lock／逐頁抓取／staging 寫入／原子切換已完成，維運端點／CLI／教學頁開發中，詳見下方「Staging Sync 快速體驗」）
...
預計的體驗流程（依 `docs/staging-sync/design.md` 設計契約，維運端點／CLI／前端頁尚未實作）：
...
4. 觀察執行紀錄：`GET /staging-sync/runs`。
5. 前端教學頁：[http://localhost:5173/staging-sync-guide](http://localhost:5173/staging-sync-guide)
   （架構圖、狀態機、即時演示）。

> 目前 schema、mock 來源 API、advisory lock、`sync_runs` 狀態機、逐頁抓取、staging 寫入、原子
> 切換已完成並可運作；上述維運端點、CLI、前端頁面依設計契約撰寫，尚在實作中，指令與路由待對應
> 模組落地後才可實際執行。
...
│   │   └── src/routes/outbox-guide.tsx   # outbox 教學頁（staging-sync-guide 開發中）
│   └── server/      # 後端 API（Fastify）
│       └── src/
│           ├── routes/    # auth.ts、todos.ts、mock-external.ts、outbox-admin.ts、mock-source.ts
