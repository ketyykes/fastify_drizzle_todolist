# Specification: transactional-outbox

## Purpose

提供 todo 完成事件的可靠外部通知能力：以 transactional outbox 模式解決「寫本地 DB」與「打外部 HTTP」無法原子化的雙寫問題——交易內只寫入待送出意圖、HTTP 送出一律在交易外進行，經 commit 後 fast-path 立即嘗試送出，失敗則交由背景 sweeper 依退避時間重試，最終保證 at-least-once 送達或轉為需人工介入的死信；並提供對應的維運端點／CLI 與前端即時觀測頁面。
